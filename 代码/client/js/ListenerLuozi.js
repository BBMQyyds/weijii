...
function normalluozi(e) {
    ...
    if (judgeNormalAble(normalabX(e.offsetX), normalabY(e.offsetY), -side)) {
        ...
        if (!judgeNormalJie(normalabX(e.offsetX), normalabY(e.offsetY), -side)) {
            ...
            document.getElementById("textarea33").innerHTML = count % 2 === 0 ? "黑棋" : "白棋";

            if (e.diy !== true) {
                //发送落子请求
                $.ajax({
                    type: "GET",
                    url: "http://101.43.255.130:8080/luozi?token=" + localStorage.getItem("token") + "&type=" + model
                        + "&other=" + e.offsetX + ":" + e.offsetY,
                    success: function (data) {
                        console.log(data);
                        data = data.replace(/[\r\n]/g, "");
                        if (data.indexOf("luozi success") !== -1) {
                            console.log('落子成功');
                        } else {
                            console.log('落子失败');
                        }
